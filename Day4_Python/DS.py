...
def baggingClassifiers():
   ...
   print('Decision Tree', accuracy_score(y_val, y_pred_tree))
...
def multiClassOneVsRestClassifier():
    ...
    _,myTrain,myVal=dataEncoding(0,taskID='filesMultiClass')
    le.fit(myVal)
    myVal=le.transform(myVal)
    myVal = label_binarize(myVal, classes=[0, 1, 2])
    n_classes = myVal.shape[1]
...
def multiClassOneVsOneClassifier():
    ...
    y_score = classifier.fit(X_train, y_train).predict(X_val)
    return y_score 
...
def multiOutputClassifier():
    ...
    pred = multi_target_forest.fit(X, Y).predict(X)
    return  pred